...
public class EnemyUnit : MonoBehaviour
{
    ...
    private void Move(float VecX)
    {
       ...
       // transform.position += MoveVec2 * MoveSpeed * Time.deltaTime;
        ...
        gameObjects = GameObject.FindGameObjectsWithTag("PlayerUnit");
    ...
    private void Attack()
    {
        ...
        if (Target != null)
        {
            ...
            if (CloseAttackUnit == true && Time.time > AttackTime + AttackCoolTime)
            {
                ...
                Target.gameObject.transform.GetComponent<PlayerUnit>().Hit(AttackDamage);
            ...
            else if (CloseAttackUnit == false && Time.time > AttackTime + AttackCoolTime)
            {
                ...
                AttackGameObject.transform.position = new Vector3(this.transform.position.x - 0.6f, 0.25f, 0f);
                attackAudioSource.Play();
                AttackTime = Time.time;
            }
        }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        {
            //int Damage = collision.gameObject.GetComponent<>
            //Hit();
        }
        //if (collision.gameObject.tag == "PlayerUnit" || collision.gameObject.tag == "EnemyUnit")
        //{
        //    MoveSpeed = 0f;
        //}
    }
